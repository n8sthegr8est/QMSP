...
#endif
//#include "msquic.h"
#include "msquiclib/include/msquic.h"
#include <stdio.h>
#include <stdlib.h>
...
//
const QUIC_REGISTRATION_CONFIG RegConfig = { "quicchat", QUIC_EXECUTION_PROFILE_LOW_LATENCY };
...
//
const uint64_t IdleTimeoutMs = 10800000;//1000;

//
// The length of buffer sent over the streams in the protocol.
//
const uint32_t SendBufferLength = 2048;
...
void PrintUsage()
{
    printf(
        "\n"
        "QUICchat runs a simple chat client or server.\n"
        "\n"
        "Usage:\n"
        "\n"
	"QUICchat -UserID:<number> -client -conAt:{IPAddress|Hostname} -chat:<number> -name:<nickname to cache>\n"
	"QUICchat -UserID:<number> -client -conTo:nickname\n"
	"QUICchat -server\n"
        );
}
    ...
    )
{
    ...
    //sentMessage((char) SendBufferRaw[0], 0, int OutOfMessagesToLoad){
    ...
    )
{
	...
	for(uint32_t i = 0; i < Event->RECEIVE.BufferCount; i++){
		unsigned char* data = (unsigned char*)Event->RECEIVE.Buffers[i].Buffer;
		uint32_t length = Event->RECEIVE.Buffers[i].Length;
		struct Message receivedMessage = unpackMessage(data,length);
		applyMessage(receivedMessage);
	}
        break;
    ...
    )
{
    ...
    const char* Cert = "./quic_certificate.pem";
    const char* KeyFile = "./quic_private_key.pem";
    ...
    )
{
    ...
    unsecure_flag = FALSE;
    Cert = "quic_certificate.pem";

    //conAt = GetTarget(argc, argv "conAt"; 
    ...
    const char* wasBoth;
    char* tempy;
    if((cidHold = GetValue(argc, argv, "UserID"))==NULL){
	    printf("Must specify a user id!\n");
	    ...
	    uid = 0;
	    if(*tempy == '-'){
		    printf("User id must be positive. Negative sign is ignored.\n");
		    tempy++;
	    }
	    while(*tempy!=0x00){
                uid *= 10;
	    ...
	    if(uid >> 62){
		    printf("User id is too large. Must be between 0 and 4,611,686,018,427,387,903\n");
		    Status = QUIC_STATUS_INVALID_PARAMETER;
		    goto Error;
	    }
    }
    if ((Target = GetValue(argc, argv, "conAt")) == NULL) {
        if((newName = GetValue(argc, argv, "conTo")) != NULL){
            struct cachedChat cached = getDetails((char*) newName);
            Target = cached.hostloc;
            cid    = cached.cid;
	}
	else{
            printf("Must specify either conAt target or conTo target!\n");
            Status = QUIC_STATUS_INVALID_PARAMETER;
            goto Error;
	}
    }
    else{
        if((wasBoth = GetValue(argc, argv, "conTo")) != NULL){
	    printf("Cannot have both conAt and conTo!\n");
            Status = QUIC_STATUS_INVALID_PARAMETER;
            goto Error;
	}
        if((cidHold = GetValue(argc, argv, "chat")) == NULL){
	    printf("Must specify a chat id to connect to at the target!\n");
	    Status = QUIC_STATUS_INVALID_PARAMETER;
	    goto Error;
	}
	if((newName = GetValue(argc, argv, "name")) == NULL){
	    printf("Chat will not be cached because no name was provided!\n");
	}
	else{
	    tempy = (char*) cidHold;
	    cid = 0;
		...
		tempy++;
            ...
            cacheNew((char*) Target, cid, (char*) newName);
	}
    }
    ...
    printf("Connected! Type logout to disconnect.\n");
    char myIn[1024];
    ...
    QUIC_BUFFER* SendBuffer;
    for(;;){
    	fgets(myIn,1024,stdin);
	if(strcmp(myIn,"logout"))
		break;
	struct DataMessageHeader dmh = constructDataMessageHeader(cid, uid, msgnum);
	msgnum++;
	unsigned long long lencount = 0;
	unsigned int       hlen = 0;
	
	lencount += getSerializedSizeOfDataMessageHeader(dmh);
	hlen = lencount;
	
	struct TransitMetadataFrame TMD;
	TMD.type = 0;
	TMD.PostTransitID = 0;
	TMD.TotalMessageCount = 1;
	TMD.SpecificMessageCount = 1;
	lencount += getSerializedSizeOfFrame(&TMD,0);

	struct PlaintextFrame* PTF;
	PTF = malloc(sizeof(struct PlaintextFrame) + strlen(myIn));
	PTF->type = 2;
	PTF->Length = 0;
	snprintf(PTF->txtdta,strlen(myIn),"%s",myIn);
	lencount += getSerializedSizeOfFrame(PTF,2);

	void** myFrames = malloc(sizeof(char*) * 2);
	myFrames[0] = &TMD;
	myFrames[1] = PTF;

	unsigned char* myMsg = SerializeDataMessageHeader(dmh);
	myMsg = PackMessage(myMsg, myFrames, hlen, 2);
	SendBuffer->Buffer = (uint8_t*) myMsg;
	SendBuffer->Length = lencount;
    ...
    Status = MsQuic->StreamSend(Connection,NULL,0,QUIC_SEND_FLAG_FIN,NULL);
