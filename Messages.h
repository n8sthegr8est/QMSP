...
struct DataMessageHeader constructDataMessageHeader(uint32_t ChatID, uint64_t SourceID, uint32_t MessageNumber);
struct ControlMessageHeader constructControlMessageHeader(char controlType, uint32_t Version, uint32_t ChatID, uint64_t SourceID, uint32_t MessageNumber, char hasData);

//struct Message constructMessage(void* messageHead,uint16_t frameCount,void* frames,...);